package com.rile.issuetracker.services.security;

import com.rile.issuetracker.entities.User;
import com.rile.issuetracker.entities.Role;
import com.rile.issuetracker.pages.Login;
...
public class PageProtectionFilter implements ComponentRequestFilter {
        ...
        // Else if user has already been authenticated (ie. already logged in)...
            ...
            // Else, redirect to the PageDenied page
                ...
                Link pageProtectedLink = pageRenderLinkSource.createPageRenderLinkWithContext(
                    Tracker.class, requestedPageName
                );
    ...
    private boolean isAuthenticated() throws IOException {
        ...
        User u = sessionStateManager.getIfExists(User.class);
        if (u != null) {
            if (u.getRole() == Role.Admin || u.getRole() == Role.Moderator ||
                u.getRole() == Role.Architect || u.getRole() == Role.Developer ||
                u.getRole() == Role.Tester || u.getRole() == Role.Member) {
                return true;
            }
        }
        return false;
    ...
    private boolean isAuthorised(RolesAllowed rolesAllowed) throws IOException {
        ...
        if (rolesAllowed == null) {
            ...
            for (String i : rolesAllowed.value()) {
                if (i.equals(sessionStateManager.getIfExists(User.class).getRole().toString())) {
                    authorised = true;
                }
            }
        }
